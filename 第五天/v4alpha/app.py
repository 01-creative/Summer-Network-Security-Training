...
def profile():
    ...
    # This allows any logged-in user to view any user's profile
    ...
    # No identity match check - session user can view ANY user's profile
    ...
    if request.method == "POST":
        ...
        # For POST operations, always use session uid to keep features working
        uid = session_uid
...
def recharge():
    # DELIBERATE VULNERABILITY: Read user_id and amount from FORM, not session
    # This allows any user to recharge/charge ANY user's account
    uid = session.get("uid")
    if not uid:
        return redirect(url_for("login"))
    ...
    # DELIBERATE VULNERABILITY: No validation for empty amount
    if not amount_str:
        return render_template("profile.html", user=user, greeting=greeting,
    ...
    logger.info(f"用户 uid={uid} 为用户 uid={target_uid} 充值: {amount}")
