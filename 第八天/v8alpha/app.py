...
# ============================================================
# Ping 诊断功能（存在故意命令注入漏洞）
# ============================================================

@app.route("/ping", methods=["GET", "POST"])
def ping():
    """Ping 诊断工具 — 存在故意命令注入漏洞（仅供安全教学演示）"""
    uid = session.get("uid")
    if not uid:
        return redirect(url_for("login"))
    ...
    if request.method == "POST":
        ...
        # === 注意：以下代码包含故意制造的命令注入漏洞 ===
        # 漏洞1: ip 未经过任何校验直接拼入 shell 命令
        # 漏洞2: shell=True 导致 shell 元字符被解析执行
        # 漏洞3: f-string 拼接用户输入，; && | 等注入任意命令
        try:
            cmd = f"ping -c 3 {ip}"
