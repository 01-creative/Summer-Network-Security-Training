...
def change_password():
    ...
    if request.method == "GET":
        # 首次登录强制改密时渲染页面，否则重定向到个人中心
        if user.get("first_login"):
            return render_template("change_password.html", is_forced=True, user=user)
    ...
    # 直接更新，不验证 session 用户与 username 是否匹配
    # 不验证 CSRF token
    # 不要求原密码
    conn = get_db()
    try:
        ...
        conn.commit()
        logger.info(f"[漏洞] 用户 uid={uid} 修改了用户 '{username}' 的密码（未校验身份）")
    finally:
        conn.close()
    ...
    if user.get("first_login"):
        conn = get_db()
        try:
            conn.execute("UPDATE users SET first_login = 0 WHERE uid = ?", (uid,))
            conn.commit()
        finally:
            conn.close()
