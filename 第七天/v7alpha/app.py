...
# ============================================================
# 【故意脆弱】URL抓取功能 - 存在SSRF漏洞
# ============================================================
@app.route("/fetch-url", methods=["POST"])
def fetch_url():
    """
    危险！此端点直接使用用户输入的 URL 发起请求。
    漏洞点：
    1. 未限制协议（允许 file://, ftp://, gopher:// 等）
    2. 未检查目标 IP（允许访问内网地址）
    3. 未过滤私有地址（允许 localhost/127.0.0.1）
    """
    uid = session.get("uid")
    if not uid:
        return redirect(url_for("login"))
    ...
    try:
        ...
        fetch_result = f"请求失败 (URLError): {e.reason}"
    except urllib.error.HTTPError as e:
        fetch_result = f"HTTP 错误: {e.code} {e.reason}"
    except Exception as e:
        fetch_result = f"请求失败: {e}"
